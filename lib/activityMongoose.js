module.exports = function (mon, options){
    ...
    var ActivityHash = {
        ...
        provider: {type: ObjectId, ref: 'activityObject'},
        generator: {type: ObjectId, ref: 'activityObject'},
        streams: [{type: String}]
    };
    ...
    this.getActivityStream = function(streamName, n, fx) {
        Activity.find({streams:streamName}).sort('published', 'descending').limit(n).populate('target').run(fx);
    }
